...
class AutoGuiApp(tb.Window):
    ...
    def create_widgets(self):
        ...
        # Control buttons
        self.buttons["registrationOnly"] = self.add_button(
            self.anchor, 1100, 250, "Auto Registration Only", self.registration_only_single
        )
        self.buttons["chargeOnly"] = self.add_button(
            self.anchor, 1100, 300, "Auto Charge Only", self.charge_only_single
        )

        self.buttons["startAutomation"] = self.add_button(
            self.anchor, self.entryPosX, 550, "Start Full Automation (Single)", self.automated_registration
        )
        self.buttons["startBatch"] = self.add_button(
            self.anchor, self.entryPosX, 650, "Start Full Automation (Batch)", self.start_batch_process
        )
        ...
        # Add button for registration only batch
        self.buttons["registrationOnlyBatch"] = self.add_button(
            self.anchor, 1100, 350, "Auto Registration Only (Batch)", self.registration_only_batch_process
        )
    ...
    def registration_only_batch_process(self):
        """Start batch process for registration only (no charging), only for rows with 'not registered' in column G."""
        if self.is_batch_running:
            messagebox.showwarning("Warning", "Batch process is already running.")
            return
        if not self.load_excel_data():
            return
        # Ensure 'Registration Status' column exists (Column G)
        status_col = None
        # Acceptable column names for status
        status_names = ["registration status", "status", "registered", "reg status"]
        for col in self.excel_data.columns:
            if str(col).strip().lower() in status_names:
                status_col = col
                break
        if not status_col:
            # Try to get column G by index if not found by name
            if len(self.excel_data.columns) >= 7:
                status_col = self.excel_data.columns[6]
            else:
                messagebox.showerror("Error", "Excel file must have a 'Registration Status' column (Column G).")
                return
        self.status_col = status_col
        self.is_batch_running = True
        self.buttons["registrationOnlyBatch"].config(state='disabled')
        self.buttons["stopBatch"].config(state='normal')
        messagebox.showinfo("Info", f"Registration-only batch started for {len(self.excel_data)} entries. Press 'ESC' to stop.")
        self.stop_automation_flag = False
        self.listener_active = True
        self.start_escape_listener()
        self.current_row_index = 0
        self.process_next_registration_only_row()

    def process_next_registration_only_row(self):
        """Process the next row for registration only, only if not registered."""
        if self.stop_automation_flag or not self.is_batch_running:
            self.finish_batch()
            return
        while self.current_row_index < len(self.excel_data):
            row = self.excel_data.iloc[self.current_row_index]
            # Defensive: handle missing or NaN status
            status = str(row[self.status_col]).strip().lower() if pd.notna(row[self.status_col]) else "not registered"
            if status == "not registered":
                ...
                print(f"[REGISTRATION ONLY] Processing Row {self.current_row_index + 1}/{len(self.excel_data)}: ID {details['idNumber']} (Status: {status})")
                try:
                    self.run_registration_only(details)
                    # After automation, move mouse and click at (921, 378)
                    pyautogui.moveTo(921, 378)
                    pyautogui.click()
                    # Update status to 'registered' in DataFrame
                    self.excel_data.at[self.current_row_index, self.status_col] = "registered"
                    ...
                    # If batch was stopped during registration, finish immediately
                    if self.stop_automation_flag or not self.is_batch_running:
                        self.finish_batch()
                        return
                    self.after(3000, self.process_next_registration_only_row)
                    return
                except Exception as e:
                    ...
                    if self.stop_automation_flag or not self.is_batch_running:
                        self.finish_batch()
                        return
                    self.after(2000, self.process_next_registration_only_row)
                    return
            else:
                print(f"[SKIP] Row {self.current_row_index + 1}: Already registered.")
                self.current_row_index += 1
        self.finish_batch()
    ...
    def save_entry_to_excel(self):
        """Appends the current GUI data as a new row to the selected Excel file, always ensuring column G ('Registration Status') is present and marks new rows as 'not registered'."""
        file_path = self.entries['excelFile'].get()
        if not file_path:
        ...
        # Prepare new data with 'Registration Status' as 'not registered'
        new_data = {
        ...
        try:
            if os.path.exists(file_path):
                ...
                # Ensure 'Registration Status' column exists
                if "Registration Status" not in existing_df.columns:
                    existing_df["Registration Status"] = "not registered"
                # Reorder columns to ensure 'Registration Status' is column G (index 6)
                cols = list(existing_df.columns)
                if "Registration Status" in cols:
                    # Move 'Registration Status' to index 6 if not already
                    cols = [col for col in cols if col != "Registration Status"]
                    cols.insert(6, "Registration Status")
                    existing_df = existing_df[cols]
                # Create new row DataFrame with same columns
                new_row_df = pd.DataFrame([[new_data.get(col, "") for col in existing_df.columns]], columns=existing_df.columns)
                updated_df = pd.concat([existing_df, new_row_df], ignore_index=True)
            else:
                # Create DataFrame with all required columns
                columns = ["ID Number", "Registration Type", "Transaction Type", "Service Type", "Company Name", "Notes/Remarks", "Registration Status"]
                new_row_df = pd.DataFrame([[new_data.get(col, "") for col in columns]], columns=columns)
    ...
    def process_next_row(self):
        ...
        if self.current_row_index < len(self.excel_data):
            ...
            try:
                self.run_full_automation(details)
                self.current_row_index += 1
    ...
    def finish_batch(self):
        ...
        self.buttons["startBatch"].config(state='normal')
        self.buttons["stopBatch"].config(state='disabled')
        self.buttons["registrationOnlyBatch"].config(state='normal')  # Re-enable registrationOnlyBatch button
    ...
    def get_gui_details(self):
        """Get details from GUI inputs"""
        return {
    ...
    def validate_details(self, details):
        ...
        if not details['idNumber']:
            messagebox.showerror("Error", "ID Number is required.")
            return False
        return True

    def setup_automation(self):
        """Setup automation environment and listener"""
        if self.is_batch_running:
    ...
    def registration_only_single(self):
        """Handle registration only for single entry from GUI"""
        details = self.get_gui_details()
    ...
    def charge_only_single(self):
        """Handle charging only for single entry from GUI"""
        details = self.get_gui_details()
        if not self.validate_details(details):
            return
        ...
        try:
            self.run_charge_only(details)
            messagebox.showinfo("Success", "Charging completed successfully!")
        except Exception as e:
            messagebox.showerror("Error", f"Charging failed: {e}")
        finally:
            self.cleanup_automation()

    def automated_registration(self):
        """Handle full automation for single entry from GUI"""
        details = self.get_gui_details()
        if not self.validate_details(details):
            return
        ...
        try:
            self.run_full_automation(details)
            messagebox.showinfo("Success", "Full automation completed successfully!")
        except Exception as e:
            messagebox.showerror("Error", f"Full automation failed: {e}")
        finally:
            self.cleanup_automation()

    def get_automation_helpers(self):
        """Get automation helper functions"""
        ...
        return coordinates, config, safe_wait, press_tab_multiple, click_at, type_text

    def handle_system_states(self, coordinates, config, safe_wait, press_tab_multiple, click_at, type_text):
        """Handle different system states based on image detection"""
    ...
    def run_registration_only(self, details):
        """Core registration logic only"""
        print(f"Starting registration only for ID: {details['idNumber']}")
        
        coordinates, config, safe_wait, press_tab_multiple, click_at, type_text = self.get_automation_helpers()

        try:
            print(f"Starting registration for ID: {details['idNumber']}")
            ...
            system_state = self.handle_system_states(coordinates, config, safe_wait, press_tab_multiple, click_at, type_text)

            if system_state == "NO_RECORD":
                return

            # If 'use existing record' is detected, continue registration as normal
            # (already handled in handle_system_states, so just continue)
            self.continue_form_registration(details, coordinates, config, safe_wait, press_tab_multiple, click_at, type_text)

        except pyautogui.FailSafeException:
            print("Registration stopped by failsafe (mouse moved to corner)")
            self.finish_batch()  # Ensure finish_batch is called
            raise

        except KeyboardInterrupt as e:
            print(f"Registration interrupted: {e}")
            self.finish_batch()  # Ensure finish_batch is called
            raise
            ...
            print(f"Registration completed for ID: {details['idNumber']}")

    def run_charge_only(self, details):
        """Core charging logic only"""
        print(f"Starting charging only for ID: {details['idNumber']}")
        
        coordinates, config, safe_wait, press_tab_multiple, click_at, type_text = self.get_automation_helpers()

        try:
            print(f"Starting charging for ID: {details['idNumber']}")
            safe_wait(config['startUpDelay'])
            
            # Handle charging services
            self.handle_charging_services(details, coordinates, config, safe_wait, press_tab_multiple, click_at, type_text)
                
        except pyautogui.FailSafeException:
            print("Charging stopped by failsafe (mouse moved to corner)")
            self.stop_batch_if_running()
            raise
            
        except KeyboardInterrupt as e:
            print(f"Charging interrupted: {e}")
            self.stop_batch_if_running()
            raise
            
        except Exception as e:
            print(f"Unexpected error in charging: {e}")
            raise
            
        finally:
            print(f"Charging completed for ID: {details['idNumber']}")

    def run_full_automation(self, details):
        """Core automation logic - registration + charging"""
        print(f"Starting full automation for ID: {details['idNumber']}")
        ...
        try:
            ...
            print(f"Full automation completed for ID: {details['idNumber']}")

    def continue_form_registration(self, details, coordinates, config, safe_wait, press_tab_multiple, click_at, type_text):
        """Continue with form registration"""
        try:
    ...
    def handle_charging_services(self, details, coordinates, config, safe_wait, press_tab_multiple, click_at, type_text):
        """Handle charging services"""
        try:
            ...
            if notes_remarks in consultation_charges:
                ...
                # Default charging for other types
                final_step()
